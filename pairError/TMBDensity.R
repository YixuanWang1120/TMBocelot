density.TMB <- function(x, c, theta, sigma, alpha, mean.x, var.obs){
  ...
  for (i in 1:n) {
    ...
    if(curr.c == K + 1){
      ...
      temp.theta <- rnorm(1, mean.x, sqrt(cof) * temp.sigma)
  ...
  for (i in 1:length(unique.class)){
    ...
    update.theta[index] <- temp.theta <- 
      rnorm(1, (mean.x + cof * sum(x[index])) / (K * cof + 1), sqrt(cof / (K * cof + 1)) * sigma[index[1]])
    ...
    update.sigma[index] <- 1 / sqrt(rgamma(1, sigma.a + length(index) / 2 + 1 / 2, sigma.b + temp / 2 + (temp.theta - mean.x)^2 / (2 * cof)))
  }
  theta <- update.theta
  sigma <- update.sigma
